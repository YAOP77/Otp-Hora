...
async function requireAdminAccessToken(req, _res, next) {
  try {
    const authHeader = req.header('authorization') || '';
    if (!authHeader.startsWith('Bearer ')) {
      return next(createError('Authorization Bearer token est obligatoire', 401, 'MISSING_TOKEN'));
    ...
    if (!user || user.status !== 'active' || user.token_version !== payload.tv || user.role !== 'admin') {
      return next(createError('Accès refusé. Privilèges administrateur requis.', 403, 'FORBIDDEN'));
    }
