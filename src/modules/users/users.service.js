...
async function createUser(payload) {
  ...
  const pin_confirmation = normalizePinInput(payload?.pin_confirmation || payload?.confirmation_pin);
  const security_questions = Array.isArray(payload?.security_questions) ? payload.security_questions : [];

  if (!nom) throw createError('Le champ nom est obligatoire', 400);
  if (!prenom) throw createError('Le champ prenom est obligatoire', 400);
  if (!username) throw createError('Le champ username est obligatoire', 400);
  
  if (username.length < 3) {
    throw createError('Le username doit contenir au moins 3 caractères', 400);
  }

  if (pin !== pin_confirmation) {
    throw createError('Les codes PIN ne correspondent pas', 400, 'PIN_MISMATCH');
  }
  ...
  if (existingUsername) {
    throw createError('Nom d’utilisateur déjà utilisé', 409, 'USERNAME_TAKEN');
  }

  if (security_questions.length < 5) {
    throw createError('Vous devez configurer au moins 5 questions de sécurité', 400, 'INSUFFICIENT_SECURITY_QUESTIONS');
  }
  ...
  // Hachage des réponses aux questions de sécurité
  const securityQuestionsData = await Promise.all(
    security_questions.map(async (sq) => ({
      question_id: randomUUID(),
      question: sq.question,
      answer_hash: await bcrypt.hash(sq.answer.trim().toLowerCase(), 10),
    }))
  );
  ...
  const createdUser = await usersRepository.createUser({
    ...
    role: ROLES.USER,
    security_questions: {
      create: securityQuestionsData,
    },
  });

  const deviceFingerprint = payload.device_meta?.device_fingerprint || randomUUID();
  await usersRepository.updateDeviceActivity(userId, deviceFingerprint, payload.device_meta || {});
  await recordUserLogin(userId, payload.device_meta || {});
...
async function updateUser(payload) {
  ...
  if (usernameProvided) {
    ...
    if (!usernameNorm) {
      throw createError('Le champ nom d’utilisateur ne peut pas être vide', 400, 'INVALID_INPUT');
    }
    ...
    const self = await usersRepository.findUserById(userId);
    if (!self) {
      throw createError('Utilisateur introuvable', 404, 'USER_NOT_FOUND');
    }
    const currentUsername = (self.username || '').toLowerCase();
    if (usernameNorm !== currentUsername) {
      const taken = await usersRepository.findUserByUsername(usernameNorm);
...
async function getSecurityQuestionsForRecovery(payload) {
  const phoneRaw = typeof payload?.phone_number === 'string'
    ? payload.phone_number
    : payload?.phone ?? payload?.contact;

  if (!phoneRaw) {
    throw createError('Le numéro de téléphone est obligatoire', 400);
  }
  ...
  if (!user) {
    throw createError('Aucun compte trouvé avec ce numéro', 404, 'USER_NOT_FOUND');
  }

  const questions = await usersRepository.findUserSecurityQuestions(user.user_id);
  
  if (!questions || questions.length === 0) {
    throw createError('Aucune question de sécurité configurée pour ce compte', 400, 'NO_SECURITY_QUESTIONS');
  }

  // On mélange les questions comme demandé dans la spec
  const shuffled = questions
    .map(q => ({ question_id: q.question_id, question: q.question }))
...
async function verifySecurityQuestionsAndResetPin(payload) {
  const userId = payload?.user_id;
  const answers = Array.isArray(payload?.answers) ? payload.answers : [];
  const newPin = normalizePinInput(payload?.pin);
  const pinConfirmation = normalizePinInput(payload?.pin_confirmation || payload?.confirmation_pin);

  if (!userId || !UUID_REGEX.test(userId)) {
    throw createError('Identifiant utilisateur invalide', 400);
  }
  ...
  const storedQuestions = await usersRepository.findUserSecurityQuestions(userId);
  
  if (answers.length !== storedQuestions.length) {
    throw createError('Toutes les réponses aux questions de sécurité sont requises', 400);
  }

  // Vérification de chaque réponse
  for (const sq of storedQuestions) {
    const userAnswer = answers.find(a => a.question_id === sq.question_id);
    if (!userAnswer) {
      throw createError('Réponse manquante pour une question', 400);
    }
    ...
    if (!isValid) {
      throw createError('Réponses de sécurité incorrectes', 401, 'INVALID_SECURITY_ANSWERS');
    }
  }

  const pin_hash = await bcrypt.hash(newPin, 12);
  await usersRepository.updateUserPinAndRotateSession(userId, pin_hash);
...
async function deactivateUserDevice(payload) {
  const userId = payload?.requester_user_id;
  const deviceId = payload?.device_id;

  if (!deviceId || !UUID_REGEX.test(deviceId)) {
    throw createError('Identifiant appareil invalide', 400);
  }

  await usersRepository.deactivateDevice(userId, deviceId);
...
module.exports = {
  ...
  verifySecurityQuestionsAndResetPin,
