...
async function registerEnterprise(payload) {
  ...
  if (!usernameNorm) {
    throw createError('Le champ nom d’utilisateur est obligatoire', 400, 'INVALID_INPUT');
  }
  ...
  const usernameTaken = await enterpriseRepository.findEnterpriseByUsername(usernameNorm);
  if (usernameTaken) {
...
async function updateEnterpriseAccount(payload) {
  ...
  if (usernameProvided) {
    ...
    const self = await enterpriseRepository.findEnterpriseByIdForAuth(companyId);
    if (!self) {
      throw createError('Entreprise introuvable', 404, 'COMPANY_NOT_FOUND');
    }
    const currentUsername = (self.username || '').toLowerCase();
    if (usernameNorm !== currentUsername) {
      const taken = await enterpriseRepository.findEnterpriseByUsername(usernameNorm);
